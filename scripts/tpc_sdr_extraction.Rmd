...
```{r, warning=FALSE, message=FALSE}
path.p<-"data/sdr/runs" #location of files
```
...
```{r, warning=FALSE, message=FALSE}
Sample.Info<-read.csv("data/sdr/SDR_Sample_Info.csv") #information on life stage, number of individuals, runs, etc.
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Resp.Data.N))){
    ...
    pdf(paste0("output/sdr/RespirationPlots/",date[i], "_",plate[i],"_",rename[j],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
write.csv(Temp.R, paste0("output/sdr/runs_temp.csv")) #save respiration rate data
```
...
write.csv(Resp.R, paste0("output/sdr/sdr_respiration_rates_raw.csv")) #save respiration rate data
...
```{r, warning=FALSE, message=FALSE}
Resp.Rates <- read.csv(file = "output/sdr/sdr_respiration_rates_raw.csv") #read file back in so slopes don't have to be generated every time
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("output/sdr/calculated_normalized_rates.csv")) #save final file
```
